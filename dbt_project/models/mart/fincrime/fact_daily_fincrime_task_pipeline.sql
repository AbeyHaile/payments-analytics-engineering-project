...
WITH pipeline AS (
    SELECT
        ...
        AVG(minutes_to_resolution) AS avg_minutes_to_resolution,
    ...
    {% if is_incremental() %}
    WHERE task_updated_at >= DATEADD('day', -3, CURRENT_DATE)
    {% endif %}
    GROUP BY
        task_created_date,
        task_type,
...
SELECT
    {{ dbt_utils.generate_surrogate_key(['task_created_date', 'task_type', 'primary_entity_type', 'resolution', 'workflow_execution_result']) }} AS kpi_report_pk,
    *,
